...
const FaceLogin: React.FC<FaceLoginProps> = ({ onClose, onSuccess }) => {
  ...
  const captureAndLogin = async () => {
    ...
    try {
      ...
      const res = await authApi.loginWithFace(dataUrl);
      const stopCamera = () => {
        const tracks = (videoRef.current?.srcObject as MediaStream | null)?.getTracks() || [];
        tracks.forEach(t => t.stop());
      ...
      if (res && res.success) {
        ...
        setMessage(res?.message || 'Xác thực khuôn mặt thất bại');
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="relative bg-white w-[420px] rounded-xl shadow-2xl px-6 py-5">
        ...
        <h3 className="text-center font-medium mb-3">Đăng nhập bằng khuôn mặt</h3>
        <div className="flex flex-col items-center gap-3">
          ...
          >
            {loading ? 'Đang xác thực...' : 'Chụp và đăng nhập'}
          </button>
